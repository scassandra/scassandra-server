...
public class CassandraExecutor22 implements CassandraExecutor {
    ...
    private <A> CassandraResult22 execute(Function<A, ResultSet> function, A input) {
        ...
        try {
            ...
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(error, message));
            ...
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(bad_credentials, e.getMessage()));
        } catch(TruncateException e) {
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(truncate_error, e.getMessage()));
        } catch(SyntaxError e) {
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(syntax_error, e.getMessage()));
        } catch(UnauthorizedException e) {
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(unauthorized, e.getMessage()));
        } catch(InvalidConfigurationInQueryException e) {
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(config_error, e.getMessage()));
        } catch(InvalidQueryException e) {
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(invalid, e.getMessage()));
        } catch(AlreadyExistsException e) {
            return new CassandraResult22(new CassandraResult.ErrorMessageStatus(already_exists, e.getMessage()));
        }
