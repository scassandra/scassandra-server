...
class HeaderTest extends FunSuite with Matchers {
  ...
  test("A Header should have the native protocol version 2") {
    val header = new Header(HeaderConsts.ClientProtocolVersion, opCode = OpCodes.Startup, streamId = stream)
    header.version should equal(HeaderConsts.ClientProtocolVersion)
  }
