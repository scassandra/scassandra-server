...
class HeaderTest extends FunSuite with Matchers {
  ...
  test("A Header should have the native protocol version 2") {
    val header = new Header(ProtocolVersions.ClientProtocolVersionTwo, opCode = OpCodes.Startup, streamId = stream)
    header.version should equal(ProtocolVersions.ClientProtocolVersionTwo)
  }
